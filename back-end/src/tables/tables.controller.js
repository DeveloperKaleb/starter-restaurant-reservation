...
function validateAttributes(req, res, next) {
  ...
  if (
    ...
    if (!tableHasGoodLength)
      invalidAttribute = invalidAttribute.concat(
        " table_name with more than 1 character"
      );
    if (!capacityIsValid)
      invalidAttribute = invalidAttribute.concat(" capacity greater than 0");
    if (!isNumber)
      invalidAttribute = invalidAttribute.concat(" capacity that is a number");
    next({
...
async function updateIsValid(req, res, next) {
  ...
  const reservation = reservation_id
    ? await reservationsService.read(reservation_id)
    : false;

  const enoughSpace =
    Number(existingTable?.capacity) >= Number(reservation?.people);
  ...
  if (
    ...
    let problem = "";
    !reservationIsNotSeated
      ? (problem = problem.concat(" unseated reservation"))
      : null;
    ...
    !enoughSpace
      ? (problem = problem.concat(
          " table with enough capacity for the reservation"
        ))
      : null;
    !tableIsNotOccupied
      ? (problem = problem.concat(" unoccupied table"))
      : null;
    next({
...
module.exports = {
  ...
  update: [
    dataExists,
    asyncErrorBoundary(updateIsValid),
    asyncErrorBoundary(update),
  ],
  deleteRecord: [
    asyncErrorBoundary(recordIsValid),
    asyncErrorBoundary(deleteRecord),
  ],
};
