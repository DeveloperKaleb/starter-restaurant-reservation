...
function dateIsDuringOpenHoursAndInTheFuture(req, res, next) {
  ...
  if (dayIsNotTues && reservationIsInTheFuture) {
    ...
    let problem = [];
    !dayIsNotTues ? problem.push(" reservation_date may not be during hours we're closed") : null;
    !reservationIsInTheFuture ? problem.push(" reservation_date and reservation_time must indicate a time in the future") : null;
...
function validateAttributes(req, res, next) {
  ...
  if ( first_name && last_name && mobile_number && reservation_date && reservation_time && people && typeof people === "number") {
    ...
    !reservation_date ? attribute = attribute.concat(" reservation_date") : null;
    !reservation_time ? attribute = attribute.concat(" reservation_time") : null;
    !people ? attribute = attribute.concat(" people") : null;
    people !== "number" ? attribute = attribute.concat("people as a number") : null;
    next({
...
module.exports = {
  create: [dataExists, validateAttributes, validateDateAndTimeFormat, dateIsDuringOpenHoursAndInTheFuture, asyncErrorBoundary(create)],
